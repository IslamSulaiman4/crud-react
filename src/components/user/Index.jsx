...
const getUsers= async()=>{
    const {data}= await axios.get('https://crud-users-gold.vercel.app/users/'); //destructing for data
    setUsers(data.users);
    setLoader(false);
}
      ...
      return(
          <React.Fragment key={user._id}>
              <tr>
                  ...
                  <td>{user.email}</td>
                  <td>{user.password}</td>
                  <td className='btn btn-danger m-1 p-2'  onClick={()=>deleteUser(user._id)}>Delete</td>
                  <td className='btn btn-info m-1 p-2' ><Link to={`/user/${user._id}`}>Details</Link></td>
                  <td className='btn btn-warning m-1 p-2'  ><Link to={`/user/edit/${user._id}`}>Edit</Link></td>
